...
if ( ! class_exists( 'wp_partita_iva_Module' ) ) {

	/**
	 * Abstract class to define/implement base methods for all module classes
	 */
	abstract class wp_partita_iva_Module {
		...
		protected static function render_template( $default_template_path = false, $variables = array(), $require = 'once' ) {
			do_action( 'wp_partita_iva_render_template_pre', $default_template_path, $variables );
			...
			if ( is_file( $template_path ) ) {
				...
				$template_content = apply_filters( 'wp_partita_iva_template_content', ob_get_clean(), $default_template_path, $template_path, $variables );
			} else {
				$template_content = '';
			}

			do_action( 'wp_partita_iva_render_template_post', $default_template_path, $variables, $template_path, $template_content );
