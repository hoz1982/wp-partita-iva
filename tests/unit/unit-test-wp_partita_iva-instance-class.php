...
require_once( dirname( dirname( __DIR__ ) ) . '/classes/wp_partita_iva-instance-class.php' );

/**
 * Unit tests for wp_partita_iva_Instance_Class
 *
 * Uses the SimpleTest For WordPress plugin
 *
 * @link http://wordpress.org/extend/plugins/simpletest-for-wordpress/
 */
if ( ! class_exists( 'UnitTestwp_partita_iva_Instance_Class' ) ) {
	class UnitTestwp_partita_iva_Instance_Class extends UnitTestCase {
		/*
		 * validate_is_valid()
		 */
		public function test_is_valid() {
			...
			foreach ( $valid_data_set as $pair ) {
				try {
					$valid = new wp_partita_iva_Instance_Class( $pair[0], $pair[1] );
					$this->pass();
			...
			foreach ( $invalid_data_set as $pair ) {
				try {
					$invalid = new wp_partita_iva_Instance_Class( $pair[0], $pair[1] );
